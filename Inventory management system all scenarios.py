# -*- coding: utf-8 -*-
"""Inventory_system_nov20_new_var_delay_2_opera (2).ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1NrAW_is_D_XtK7mgM4hQAp0qROLVi0Rk
...
df_comparison_SC=empty_inventory_data_result.copy()

df_comparison_SC

protocol_coef=4
service_level = 0.9900
...
df_comparison_SC=df_comparison_SC.append([inventory_data_result])
#df_comparison=data_diff_coef.copy()
df_comparison_SC

#['forecast with %d days'%end_day_training]
df_comparison_SC.to_csv('result comparison different service level and coef 45.csv')#%protocol_coef)

df_comparison_SC=pd.read_csv('result comparison different service cycle.csv')

#run this first and once
data_diff_coef_SC=inventory_data_result.copy()
df_comparison_SC=data_diff_coef_SC.copy()
df_comparison_SC
...
#df_comparison_SC_chart=df_comparison_SC[['Forecast method','Shortage','Service level - specified']]
df_comparison_SC_presentation=df_comparison_SC[['Forecast method','Shortage','Service level - specified','Service level - achieved','Number of orders','Real cost','Number of epochs with shortage']]
#df_comparison_SC_present=df_comparison_SC.copy()
df_comparison_SC_presentation

df_comparison_SC_presentation['Shortage']=df_comparison_SC_presentation['Shortage'].astype(str).replace('\.0', '', regex=True)
df_comparison_SC_presentation['Service level - specified']=df_comparison_SC_presentation['Service level - specified'].astype(str).replace('\.0', '', regex=True)
#df_comparison_shortage_presentation['Real cost']=df_comparison_shortage_presentation['Real cost'].astype(str).replace('\.0', '', regex=True)
df_comparison_SC_presentation['Real cost'] = df_comparison_SC_presentation['Real cost'].map('${:,.2f}'.format)

df_comparison_SC_presentation.set_index(['Service level - specified','Forecast method'])

"""### Shortage chart"""

df_comparison_SC_chart=df_comparison_SC[['Forecast method','Shortage','Service level - specified']]
df_comparison_SC_chart
...
chart=sns.barplot(x="Service level - specified", 
            y="Shortage", 
            hue="Forecast method", 
            data=df_comparison_SC_chart,palette=["C0", "C1", "C2"])
...
plt.savefig('Total number of shortage with different Service levels and coef of 45',dpi=300)#%protocol_coef,dpi=300)

"""### Real cost chart"""

df_comparison_SC_real_cost=df_comparison_SC[['Forecast method','Real cost','Service level - specified']]
df_comparison_SC_real_cost
...
plt.savefig('Real cost with different Service levels and coef of 45',dpi=300)#%d'%protocol_coef,dpi=300)
...
df_comparison_coeff=df_comparison_coeff.append([inventory_data_result])
#df_comparison=data_diff_coef.copy()
df_comparison_coeff

df_comparison_coeff.to_csv('result comparison different coeff and service lvl 99.csv')

#run this first and once
data_diff_coef=inventory_data_result.copy()
df_comparison_coeff=data_diff_coef.copy()
df_comparison_coeff
...
#Different COEFFICIENT OF CONSUMPTION

#df_comparison_SC_chart=df_comparison_SC[['Forecast method','Shortage','Service level - specified']]
df_comparison_coeff_presentation=df_comparison_coeff[['Forecast method','Shortage','Protocol coefficient','Service level - achieved','Number of orders','Real cost','Number of epochs with shortage']]
#df_comparison_SC_present=df_comparison_SC.copy()
df_comparison_coeff_presentation

df_comparison_coeff_presentation['Shortage'] = df_comparison_coeff_presentation['Shortage'].astype(str).replace('\.0', '', regex=True)
df_comparison_coeff_presentation['Protocol coefficient']=df_comparison_coeff_presentation['Protocol coefficient'].astype(str).replace('\.0', '', regex=True)
df_comparison_coeff_presentation['Service level - achieved']=df_comparison_coeff_presentation['Service level - achieved'].astype(str).replace('\.0', '', regex=True)
df_comparison_coeff_presentation['Real cost'] = df_comparison_coeff_presentation['Real cost'].map('${:,.2f}'.format)

df_comparison_coeff_presentation.set_index(['Protocol coefficient','Forecast method'])

"""### Shortage chart"""

df_comparison_coeff_chart=df_comparison_coeff[['Forecast method','Shortage','Protocol coefficient']]
df_comparison_coeff_chart
...
chart=sns.barplot(x="Protocol coefficient", 
            y="Shortage", 
            hue="Forecast method", 
            data=df_comparison_coeff_chart,palette=["C0", "C1", "C2"])
...
plt.savefig('Total number of shortage with different protocol coefficient and serivce lvl 99',dpi=300)#%protocol_coef,dpi=300)

"""### Real cost chart"""

df_comparison_coeff_real_cost=df_comparison_coeff[['Forecast method','Real cost','Protocol coefficient']]
df_comparison_coeff_real_cost
...
plt.savefig('Real cost with different protocol coefficient and service lvl 99',dpi=300)#%protocol_coef,dpi=300)

#df_comparison_shortage = df_comparison_shortage.set_index(['Protocol coefficient', 'Forecast method']).Shortage

#df_comparison_shortage.unstack().plot(kind='bar',stacked=False)
#fig.set_size_inches(10,5)
#fig = plt.df_comparison_shortage.unstack().plot(kind='bar').(figsize=(17,8.5))

#putting the simulated data into the dataframe
hospitalization_data_SIR_forecast['forecast with %d days'%end_day_training]=estimated_data
hospitalization_data_SIR_forecast.head()

"""## Miscellaneous test sections"""

y_test=y[:16].copy()

number_of_epoch_in_original=math.floor(len(y_test)/R)
number_of_epoch_in_original
...
def noise_creator_on_overall_consumption(data_without_noise,mean_type):
  ...
  if mean_type=='mean_each_epoch':
    ...
    for i in range(math.ceil(len(data_without_noise)/R)):
      ...
      #creating noise with normal distribution
      noise=np.random.normal(0, sigma_noise, temp_data.shape) 
      temp_data=temp_data+noise
    ...
    data_with_noise = data_without_noise[0:0]
    sigma_noise = max_of_data_without_noise * percentage_of_variation
    noise=np.random.normal(0, sigma_noise, data_without_noise.shape)
    data_with_noise=data_without_noise+noise
    data_with_noise[data_with_noise<0]=0
...
y_test1=noise_creator_on_overall_consumption(y_test,mean_type_for_noise)
y_test1
...
data_with_noise=y_test[0:0]
sigma_noise=mean_of_test_data*percentage_of_variation
noise=np.random.normal(0, sigma_noise, y_test.shape)
data_with_noise=y_test+noise
data_with_noise[data_with_noise<0]=0

data_with_noise

percentage_of_variation=1
mu_noise=0
#Choose one of the following methods for the noise calculation
#       mean_each_epoch
#       max_each_epoch
#       max_all_epoch
#       mean_all_epoch
mean_type_for_noise='mean_all_epoch'

data_with_noise=y_test[0:0]
max_of_test_data=y_test.max()
mean_of_test_data = y_test.mean()

#dividing the length of data by the epoch period will provide with the number of epochs in the horizon
#we are rounding it to accomodate for days after the last epoch
for i in range(math.ceil(len(y_test)/R)):
  dummy_y=y_test[i*R:(i+1)*R]
  mean_val=dummy_y.max()
  sigma_noise=mean_val*percentage_of_variation
  noise=np.random.normal(0, sigma_noise, dummy_y.shape) 
  dummy_y=dummy_y+noise
  dummy_y[dummy_y<0]=0
  data_with_noise=data_with_noise.append(dummy_y)
  print(noise)
#data_with_noise
...
dumpy_y=y_test[:1*R]
mean_val=dumpy_y.mean()
percentage_of_variation=1
mu_noise=0
sigma_noise=mean_val*percentage_of_variation
noise=np.random.normal(0, sigma_noise, dumpy_y.shape) 
dumpy_y=dumpy_y+noise
dumpy_y[dumpy_y<0]=0
dumpy_y

dumpy_y1=dumpy_y.copy()
dumpy_y1

dumpy_y2=y_test[1*R:2*R]
dumpy_y2

dumpy_y2=y_test[1*R:2*R]
mean_val=dumpy_y2.mean()
percentage_of_variation=1
mu_noise=0
sigma_noise=mean_val*percentage_of_variation
noise=np.random.normal(0, sigma_noise, dumpy_y2.shape) 
dumpy_y2=dumpy_y2+noise
dumpy_y2[dumpy_y2<0]=0
dumpy_y2

test1=dumpy_y1.append(dumpy_y2)
test1

mu, sigma = 0, 0.1 
# creating a noise with the same dimension as the dataset (2,2) 
noise = np.random.normal(mu, sigma, test.shape) 
noise

with_noise=test+noise
with_noise
